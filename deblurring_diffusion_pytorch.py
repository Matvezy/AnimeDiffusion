...
class GaussianDiffusion(nn.Module):
    ...
    def get_conv(self, dims, std, mode='circular'):
        kernel = torch.tensor([[0.0, 0.0, 0.0], [0.0, 1.001, 0.0], [0.0, 0.0, 0.0]])
    ...
    """
    def get_conv_sobel(self, dims, std, mode='circular'):
        kernel = self.blur(dims, std)
        conv = nn.Conv2d(in_channels=self.channels, out_channels=self.channels, kernel_size=dims, padding=int((dims[0]-1)/2), padding_mode=mode,
                         bias=False, groups=self.channels)
        with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            #G = G.unsqueeze(1)
            Gx = Gx.repeat(self.channels, 1, 1, 1)
            conv.weight = nn.Parameter(Gx)
        return conv
    """
    def green_transf():
        img =  cv2.imread("CelebA-img/0.jpg").astype(np.float)  # BGR, float
        for i in range(255):
            r = np.absolute(img[:, :, 2] + 1)
            img[:, :, 2] = np.clip(r, 0, 255)
            b = np.absolute(img[:, :, 1] - 1)
            img[:, :, 1] = np.clip(b, 0, 255)
            g = np.absolute(img[:, :, 0] - 1)
    ...
    def get_kernels_sobel(self):
        kernels = []
        for i in range(self.num_timesteps):
            if self.blur_routine == 'Incremental':
                kernels.append(self.get_conv_lapl_gaus((self.kernel_size, self.kernel_size)))
            elif self.blur_routine == 'Constant':
                kernels.append(self.get_conv_lapl_gaus((self.kernel_size, self.kernel_size)))
            elif self.blur_routine == 'Constant_reflect':
                kernels.append(self.get_conv_lapl_gaus((self.kernel_size, self.kernel_size), mode='reflect') )
            elif self.blur_routine == 'Exponential_reflect':
                ks = self.kernel_size
                kernels.append(self.get_conv_lapl_gaus((ks, ks), mode='reflect'))
            elif self.blur_routine == 'Exponential':
                ks = self.kernel_size
                kernels.append(self.get_conv_lapl_gaus((ks, ks)))
            elif self.blur_routine == 'Individual_Incremental':
                ks = 2*i+1
                kernels.append(self.get_conv_lapl_gaus((ks, ks)))
            elif self.blur_routine == 'Special_6_routine':
